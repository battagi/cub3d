...
/*   Updated: 2025/04/22 17:29:40 by abattagi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vars	assign_hvars(t_rays *ray, t_game *data, float ang)
{
	...
	if (ang_down(ang))
		vars.yintercept += TILE_SIZE;
	vars.xintercept = (int)data->player->pos_x + (vars.yintercept
			- (int)data->player->pos_y) / tan(ang);
...
t_vars	assign_vvars(t_rays *ray, t_game *data, float ang)
{
	...
	if (ang_to_right(ang))
		vars.xintercept += TILE_SIZE;
	vars.yintercept = (int)data->player->pos_y + (vars.xintercept
			- (int)data->player->pos_x) * tan(ang);
...
void	calculate_hdist(t_game *data, t_rays *ray)
{
		...
		&& ray->hwallhity < data->map_height)
	{
		if (ray->hwallhitx >= 0 && ray->hwallhity >= 0
			&& ray->hwallhitx < data->map_width
			&& ray->hwallhity < data->map_height
			&& (data->map[(int)ray->hwallhity][(int)ray->hwallhitx] == '1'
				|| data->map[(int)ray->hwallhity][(int)ray->hwallhitx] == 'D'))
		{
			check_h(calcul_dist(data->player->pos_x, data->player->pos_y,
						ray->horz_v.xintercept, ray->horz_v.yintercept)* cos(ray->ang - data->player->dir), data,
				(int)ray->hwallhity, (int)ray->hwallhitx);
			...
			ray->hdist = calcul_dist(data->player->pos_x, data->player->pos_y,
				...
				* cos(ray->ang - data->player->dir);
				...
				{
				ray->hdist = calcul_dist(data->player->pos_x, data->player->pos_y,
						ray->horz_v.xintercept, ray->horz_v.yintercept)* cos(ray->ang - data->player->dir);
...
void	calculate_vdist(t_game *data, t_rays *ray)
{
		...
		&& ray->vwallhity < data->map_height)
	{
		if (ray->vwallhitx >= 0 && ray->vwallhity >= 0
			&& ray->vwallhitx < data->map_width
			&& ray->vwallhity < data->map_height
			&& (data->map[(int)ray->vwallhity][(int)ray->vwallhitx] == '1'
				|| data->map[(int)ray->vwallhity][(int)ray->vwallhitx] == 'D'))
		{
			check_v(calcul_dist(data->player->pos_x, data->player->pos_y,
					ray->vert_v.xintercept, ray->vert_v.yintercept), data,
				(int)ray->vwallhity, (int)ray->vwallhitx);
			...
			{
				ray->vdist = calcul_dist(data->player->pos_x, data->player->pos_y,
					ray->vert_v.xintercept, ray->vert_v.yintercept) * cos(ray->ang - data->player->dir);
			}
			break ;
		}
